        ...
        for (x, y, w, h) in rostos_detectados:
            ...
            if confianca < LIMITE_CONFIANCA and rotulo_id < len(ids_treinamento):
                ...
                if status == "Autorizado":
                    ...
                    if tempo_restante > 0:
                        texto_timer = f"Abrindo em {tempo_restante}s..."
                        ...
                        if nivel == "Nivel 3":
                            try:
                                print("Iniciando o aplicativo de administração...")
                                subprocess.Popen([sys.executable, os.path.join(base_dir, 'admin_app.py')])
                            except Exception as e:
                                print(f"Erro ao iniciar admin_app.py: {e}")
                        else:
